...
@:allow(h2d.animate.internal.Frame)
class Layer
{
	 ...
	 * @return						A ``Bounds`` with the layer's bounds at an index, empty if no elements were found.
	 */
	public function getBounds(frameIndex:Int, ?bounds:Bounds, ?matrix:Matrix, ?includeFilters:Bool = true, ?useCachedBounds:Bool = false):Bounds
	{
		...
		if (frame != null)
			return frame.getBounds((frameIndex - frame.index), bounds, matrix, includeFilters, useCachedBounds);
	...
	function _loadJson(layer:LayerJson, parent:AnimateLibrary, ?layerIndex:Int, ?layers:Array<Layer>):Void
	{
		this.name = layer.LN;
