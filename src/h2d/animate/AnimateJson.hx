...
extern abstract AnimationJson(Dynamic)
{
	...
	public var SD(get, never):Null<Vector<SymbolJson>>;
...
extern abstract FrameJson(Dynamic)
{
	...
	public var B(get, never):Null<AnimateBlendMode>;
...
abstract FilterJson(Dynamic)
{
	...
	function getGradientArray():{colors:Array<Int>, alphas:Array<Float>, ratios:Array<Float>}
	{
		...
		for (entry in GE)
		{
			colors.push(Color.fromString(entry.C));
			alphas.push(entry.A);
			ratios.push(entry.R);
		}
	...
	public function to2DFilter():Filter
	{
		switch (this.N)
		{
			case "blurFilter" | "BLF":
				trace('Filter with name "${this.N}" is not currently supported on this target.');
				// return new Blur(BLX, Q);
				return null;

			case "adjustColorFilter" | "ACF":
                var m = new h3d.Matrix();
                m.identity();
                m.colorLightness(BRT);
                m.colorHue(H);
                m.colorContrast(CT);
                m.colorSaturate(SAT);
			...
			case "dropShadowFilter" | "DSF":
				trace('Filter with name "${this.N}" is not currently supported on this target.');
				// var dsf = new DropShadow(D, AL, Color.fromString(C), A, BLX, STR, Q, IN, KK, HO);
				// return dsf;
                return null;

			case "glowFilter" | "GF":
				trace('Filter with name "${this.N}" is not currently supported on this target.');
				// var gf = new Glow(Color.fromString(C), A, BLX, STR / 100, Q, IN, KK);
				// return gf;
                return null;
...
abstract MatrixJson(Array<Float>) from Array<Float>
{
	...
	extern inline public function toMatrix():Matrix
	{
...
enum abstract AnimateBlendMode(Int) {
	public var ADD = 0;
	public var ALPHA = 1;
	public var DARKEN = 2;
	...
	@:from private static function fromString(value:String):AnimateBlendMode
	{
		return switch (value)
		{
			case "add": ADD;
	...
	{
		return switch (cast this : AnimateBlendMode)
		{
			case AnimateBlendMode.ADD: "add";
			case AnimateBlendMode.ALPHA: "alpha";
			case AnimateBlendMode.DARKEN: "darken";
			case AnimateBlendMode.DIFFERENCE: "difference";
			case AnimateBlendMode.ERASE: "erase";
			case AnimateBlendMode.HARDLIGHT: "hardlight";
			case AnimateBlendMode.INVERT: "invert";
			case AnimateBlendMode.LAYER: "layer";
			case AnimateBlendMode.LIGHTEN: "lighten";
			case AnimateBlendMode.MULTIPLY: "multiply";
			case AnimateBlendMode.NORMAL: "normal";
			case AnimateBlendMode.OVERLAY: "overlay";
			case AnimateBlendMode.SCREEN: "screen";
			case AnimateBlendMode.SHADER: "shader";
			case AnimateBlendMode.SUBTRACT: "subtract";
			default: "normal";
		}
	}
}
